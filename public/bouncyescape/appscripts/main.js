...
if (paper.height/5*4 > paper.width){ //if pWidth restricts the ratio
    var pWidth = paper.width
    var pHeight = paper.width/4*5 //set pHeight according to paper width

} else { // if pHeight restricts the ratio
...
var startButton = paper.rect(pWidth/2-buttonWidth/2,pHeight/2-buttonHeight/2,buttonWidth,buttonHeight);
startButton.attr({"fill":"grey"});
var startText = paper.text(pWidth/2,pHeight/2,"START").attr({"font-family":"Trebuchet MS", "font-size": 20});

var welcomeText= paper.text(pWidth/2,pHeight/3,"Bouncy Escape").attr({"font-family":"Trebuchet MS", "font-size": 20, "fill":"white"});
var controlText1= paper.text(pWidth/2,pHeight/4*3-30,"Controls:").attr({"font-family":"Trebuchet MS", "font-size": 16, "fill":"white", "opacity":0.6});
var controlText2= paper.text(pWidth/2,pHeight/4*3,"Left/Right Arrow Keys to move sideways").attr({"font-family":"Trebuchet MS", "font-size": 12, "fill":"white", "opacity":0.6});
var controlText3= paper.text(pWidth/2,pHeight/4*3+16,"Spacebar to jump").attr({"font-family":"Trebuchet MS", "font-size": 12, "fill":"white", "opacity":0.6});
var controlText4= paper.text(pWidth/2,pHeight/4*3+40,"Tip: Press and hold the spacebar at times!").attr({"font-family":"Trebuchet MS", "font-size": 10, "fill":"white", "opacity":0.6});
...
var endGameLose = function (){
    ...
    var loseScreen = paper.rect(0,0,pWidth,pHeight).attr({"fill":"white", "fill-opacity": 0.6, "stroke-width":0})
    var loseText = paper.text(pWidth/2,pHeight/2-90,"Game Over").attr({
        "font-family": "Trebuchet MS", "font-size":25});
    var loseText2 = paper.text(pWidth/2,pHeight/2-50,"Your score was: "+score).attr({ //inform players of score
...
var showRestartButton = function(){
    var restartButton= paper.rect(pWidth/2-buttonWidth/2,pHeight/2-buttonHeight/2,buttonWidth,buttonHeight);
    restartButton.attr({"fill":"grey"});

    var restartText = paper.text(pWidth/2,pHeight/2,"RESTART").attr({
        "font-family":"Trebuchet MS", "font-size": 20})
...
var showPauseButton=function(){
    var pauseBg = paper.rect(10, 10, 30, 30, 5).attr({
        "fill": "#f2fbfd","opacity": 0.3});

    var line1 = paper.rect(17,17,5,16,1).attr({
        "fill": "#f2fbfd", "stroke":"#f2fbfd", "opacity": 0.9});
...
var gamePause = function(){
    pauseScreen = paper.rect(0,0,pWidth,pHeight).attr({
        "fill":"white", "fill-opacity": 0.6, "stroke-width":0})
...
var showResumeButton = function(){
    resumeButton= paper.rect(pWidth/2-buttonWidth/2,pHeight/2-buttonHeight/2,buttonWidth,buttonHeight);
...
var diffAdj = function(){
    if (score>14 && (score % 5 ===0) && platWidth>30){ //from 15 onwards, every 5 platforms until platwidth=30,
...
var createPlatforms = function(platNum){
    ...
    while (i<=platNum){
        ...
        platforms[i]= paper.rect(newX, startHeight-heightChange*i, platWidth, platHeight)
...
var createWater = function(){
    ...
    var waterflash1 = function(){water.animate({"fill":"white", "fill-opacity":0.8},700, waterflash2)}
    var waterflash2 = function(){water.animate({"fill":"cyan", "fill-opacity":0.4}, 700, waterflash1)}
    waterflash1();
...
var ballGravity = function(){
    ...
    for (let i =0; i<platforms.length; i++){    //check through entire array
        if (ball.attr('cy')<= platforms[i].y){  //if ball is above platform, add platform to platformsUnder
    ...
    platformsUnder.forEach(plat =>{
        ...
        } else {ballState="falling";} // when fall off boundary, change back to falling and trigger GRAVITY
    });
};
...
var updateProgress = function(){
    ...
    if (score> highScore){
        ...
        if (highScoreState!="broken" //set if condition based on highscore state to play audio ONCE upon breaking highscore per game
            && highScore!=1){ //set second if condition to play hscoreAudio only when player has broken his own hscore (ie. not upon first game)
            hscoreAudio.play();
            hscoreAudio.volume=soundVolume.value;

            //if break highscore in a game, create "New Highscore!" text for player visual feedback
            var hscoreText = paper.text(pWidth/2, pHeight/4, "New Highscore!").attr({
                "font-family": "Trebuchet MS", "font-size":14, "stroke":"white", "fill":"white"})
        ...
        platforms.forEach(plat =>{
            ...
            if (plat.y>=pHeight){
                ...
                if (plat.outOfScreen===false){
                    ...
                    newPlatform=paper.rect(newX, plat.y-heightChange*6, platWidth, platHeight);//create new platform 
                    ...
                    console.log("plat remove is"+plat.index+ "newPlat created is "+ newPlatform.index+ "at y= "+ newPlatform.y)
                }
            };
        });

        //MOVE WATER LEVEL according to movement of platforms
        if (water.attr('y')<pHeight*0.98){ //check water above set limit
            if (water.y+heightChange<pHeight*0.98){ //if water level lowers by full heightChange is still within set limit,
            ...
            } else { //if water level above set limit BUT changing by heightChange will make it exceed set limit,
...
document.addEventListener("keyup", function(ev){
    if (ev.which===37){keys[37]=false};
    if (ev.which===39){keys[39]=false};
    if (ev.which===32){keys[32]=false};
});
...
var mainLoop= function(){
    ...
    if (ball.attr('cx')<=rad){ //if touch left screen limit, keep ball there
        ball.posX=10 
    } else if (ball.attr('cx')>=pWidth-rad){ //if touch right screen limit, keep ball there
        ball.posX=pWidth-10}; 

    if(ball.attr('cy')<=0){ //if hit top of screen, keep ball there
        ball.posY=0
    } 


    //check for ENDGAME 
    if (water.attr('y')<=ball.attr('cy')+rad){ // if ball touches water level
        endGameLose();
    }
};
